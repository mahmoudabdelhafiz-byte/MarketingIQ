...
def normalize_domain(value: str) -> str:
    ...
    try:
        ...
        raise ValueError("domain contains an invalid port") from error
...
class CompanyService:
    ...
    def attach_company(
        ...
        *,
        ...
        if company is None:
            values = {key: value for key, value in (shared_values or {}).items() if value is not None}
        ...
        if relationship is None:
            ...
            self._commit()
        return self.get_company(relationship.id)
    ...
    def list_sources(self) -> list[DataSource]:
        ...
        return list(
            self.session.scalars(
                select(DataSource)
                .where(DataSource.provider_key.in_(sorted(ALLOWED_SOURCE_KEYS)))
                .order_by(DataSource.display_name)
            ).all()
        )
    ...
    def add_fact(self, relationship_id: str, data: FactInput) -> CompanyFact:
        ...
        if (
            ...
            raise ValueError("third-party licensed facts cannot be submitted as ALLOWED")
        ...
        if data.observed_at is not None:
            fact.observed_at = data.observed_at
        for item in data.evidence:
            if item.data_source_id is not None:
                source = self.session.get(DataSource, item.data_source_id)
                if source is None or source.provider_key not in ALLOWED_SOURCE_KEYS:
                    raise ValueError("unknown data source")
            evidence = Evidence(
                data_source_id=item.data_source_id,
                reference_url=(validate_reference_url(item.reference_url) if item.reference_url else None),
                reference_text=item.reference_text,
                last_verified_at=item.last_verified_at,
            )
            if item.retrieved_at is not None:
                evidence.retrieved_at = item.retrieved_at
    ...
    def preview_csv(self, content: str | bytes) -> ImportReport:
        require_permission(self.tenant, Permission.IMPORT_COMPANIES)
    ...
    def _preview_row(self, number, values, existing, virtual_new, virtual_attached) -> ImportRow:
        ...
        if errors:
            ...
            if company_id in virtual_attached:
                ...
                warnings.append("duplicate or already attached domain; row values will not overwrite")
            elif company_id:

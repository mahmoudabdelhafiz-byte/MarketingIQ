...
def make_tenant(session, slug: str, role=MembershipRole.ORGANIZATION_ADMIN):
    ...
    session.flush()
    return organization, user, CompanyService(session, TenantContext(organization.id, user.id, role))
...
def test_facts_are_append_only_provenanced_and_tenant_scoped(session):
    ...
    fact_ids = [fact.id for fact in first.list_facts(first_relationship.id)]
    assert january.id in fact_ids and september.id in fact_ids
...
def test_provenance_and_redistribution_rules(session):
    ...
    with pytest.raises(ValueError, match="cannot be submitted as ALLOWED"):
        service.add_fact(
            relationship.id,
            FactInput(
                fact_key="industry",
...
def test_csv_preview_is_non_mutating_and_import_handles_duplicates(session):
    ...
    assert session.scalar(select(func.count()).select_from(OrganizationCompany)) == before_relationships
    ...
    attached = service.list_companies()
    shared = next(item for item in attached if item.company_id == existing.company_id)
    assert shared.private_notes == "tenant note"
